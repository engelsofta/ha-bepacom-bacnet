...
Während des Reloads können Entitäten kurz nicht verfügbar sein.`
  ...
  _mainStatusHtml() {
    ...
    const connectedLabel = diagnostics.connected === void 0 ? "-" : diagnostics.connected ? "Ja" : "Nein";
  ...
  _effectiveTransportPresentation(point) {
    ...
    const settled = !["unknown", "waiting"].includes(effectiveMode) && !["subscribing", "cov_waiting", "polling_waiting"].includes(effectiveState);
    return {
      state,
      label,
      detail,
...
let BepacomRuntimeDashboard = class extends i {
  ...
  _diagnosticPipeline() {
    ...
    const step = (label, value, hint, icon, tone = "") => b`
