# Pre-release checklist for 1.2.1b1

## Before publishing

- [x] `manifest.json` version is `1.2.1b1`.
- [x] `const.py` version is `1.2.1b1`.
- [x] README version badge is `1.2.1b1`.
- [x] Changelog contains a dated `1.2.1b1` entry.
...
3. Create the GitHub release with tag `1.2.1b1` targeting the release commit.
4. Use `Engelsoft Beacon BACnet/IP 1.2.1 B1` as the release title.
...
7. Verify that HACS detects version `1.2.1b1` when beta releases are enabled.

## After publishing

- [ ] Install/update through HACS on a test system.
- [ ] Restart Home Assistant and bypass the browser cache.
- [ ] Confirm integration version `1.2.1b1` and frontend build `0649`.
